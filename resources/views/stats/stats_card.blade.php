@php
    ...
    $actGithubLogo = $data['active_git_logo'];
    $hideItems = ($additinlStats === "true") ? null : $data['hideItems'];
    ...
    if ($hideItems !== null && $hideItems !== "full") {
        foreach ($hideItems as $hideItm) {
            unset($statCardItmDt[$hideItm - 1]);
        }
        $statCardItmDt = array_values($statCardItmDt);
        if (count($statCardItmDt) <= 2) {
            $cardHeight = (2 * 20)+105;
        } else {
            $cardHeight = (count($statCardItmDt) * 20)+105;
        }
    }
    elseif ($hideItems === "full") {
        array_splice($statCardItmDt, 0);
    }

@endphp
<svg xmlns="http://www.w3.org/2000/svg" width="450" height="{{ !empty($statCardItmDt) ? $cardHeight : 185 }}" viewBox="0 0 450 {{ !empty($statCardItmDt) ? $cardHeight : 185 }}" fill="#000" role="img" aria-labelledby="descId">
    ...
    <rect class="card-bg" x="0.5" y="0.5" rx="4.5" height="calc(100% - 1px)" width="{{!empty($statCardItmDt) ? 449 : 280}}" stroke-opacity="1"/>
    <g data-testid="card-title" transform="translate(25, 35)">
        <g transform="translate(0, 0)">
            @if ($hideItems === "full")
                <text x="0" y="0" class="header" data-testid="header">{{ $data['title'] }} Github Rank</text>
            @else
    ...
    <g data-testid="main-card-body" transform="translate(0, 55)">
        <g data-testid="rank-circle" transform="translate({{ !empty($statCardItmDt) ? 365 : 140 }}, {{ !empty($statCardItmDt) ? ((count($statCardItmDt)/2)*20) : 45}}) {{ !empty($statCardItmDt) ? '' : ', scale(1.15)' }}">
            <circle class="rank-circle-rim" cx="-10" cy="8" r="40"/>
            <circle class="rank-circle" cx="-10" cy="8" r="40"/>
            <g class="rank-text">
                @if ($actGithubLogo === "true")
